...
enemy_hits_bullet = False

def reset():
    global GAME_OVER, bullet_pos, bullet_direction, bullet_inflight, enemy_hits_bullet, enemy_pos, enemy_box
    GAME_OVER = False
    bullet_pos=player_pos.copy()
    bullet_direction= None
    bullet_inflight=False
...
while running:
    ...
    if enemy_box is not None and enemy_box.colliderect(player_box):
        GAME_OVER =True
    ...
    for wall in walls:
        ...
        if  enemy_box is not None and wall_box.colliderect(enemy_box):
    ...
    if bullet_pos.x < 0 or bullet_pos.x>640 or bullet_pos.y<0 or bullet_pos.y>480:
        ...
        bullet_direction= None
        bullet_inflight=False
    if  enemy_box is not None and bullet_box is not None and bullet_box.colliderect(enemy_box):
        enemy_hits_bullet = True
        GAME_OVER = True
      
        
    if enemy_hits_bullet==False:
        enemy_box = pygame.draw.rect(screen,"green" ,[enemy_pos.x,enemy_pos.y,30,30])
       
    if GAME_OVER:
        MSG= "YOU WIN" if enemy_hits_bullet == True  else'GAME OVER'
